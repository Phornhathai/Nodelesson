...
const   db_protocol     = `mongodb://`,
        db_path         = `/admin`,
        db_host         = `mongo`,     
        db_port         = `:27017`,  //27017(internal port) is default port in mongoDB : Now connect from external by configure in docker. If when use in server, it will create new container by port 27017 and ip host will change (name of container instead of ip no.)
...
async function update (dbname , collectionName , data , filter={}, operator="set"){
    ...
    try{
        ...
        let options   = {
            ...
            let updateData = {}
            switch(operator){
                case     "push"     : updateData  = { $push    :   data }  
                    break
                case     "pull"     : updateData  = { $pull    :   data }  
                    break
                case     "pullAll"  : updateData  = { $pullAll :   data }  
                    break
                default             : updateData  = { $set     :   data }  
                    break
            }
